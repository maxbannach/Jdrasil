...
\end{enumerate}
The selection of \JClass{SATDecomposer} and \JClass{CopsAndRobber} is
also separated in the class \JClass{ExactDecomposer}.
...
have to implement a class \JClass{ColoringStateVector} that implements the \JClass{StateVector}
...
\JClass{jdrasil.utilities.sat.Formula}, which represents a boolean formula in
...
\JClass{jdrasil.utilities.sat.SAT4JSolver}, which is a
\JClass{jdrasil.utilities.sat.ISATSolver}. If the SAT4J library is found in
\Jdrasil's classpath, this class will be used by
\JClass{jdrasil.utilities.sat.Formula} (see~\ref{section:satFormula}) to find a
...
\JMethod(canRegisterSATSolver()) of \JClass{jdrasil.utilities.sat.Formula} will
...
\JClass{jdrasil.utilities.sat.NativeSATSolver}, which implements the interface
\JClass{jdrasil.utilities.sat.ISATSolver} with native methods. Note how this
interface, which we also use for other solvers like SAT4J, is closely
related to IPASIR. 

The corresponding C interface of \JClass{jdrasil.utilities.sat.NativeSATSolver} can be found
