...
to allow other projects to add tree decompositions to there
...
principle time complexity anyway. If we, however, which to use super
...
If you use \Jdrasil\ for your project, please reference our GitHub
...
well thought out representation of such decompositions is
...
method to create new nodes in the decomposition and to link existing nodes.
...
not have any in formations about the decomposition stored yet.
\begin{lstlisting}[language=Java]
  // read the graph G
...
\end{lstlisting}
And that's it. We have successfully created a tree
...
should rather be used on already good decompositions (and not on the
...
The class \JClass{DegeneracyLowerbound} implements the linear time algorithm from Matula and Beck
to compute the degeneracy of a graph.
...
The class \JClass{ImprovedGraphLowerbound} implements the improve graph trick to improve some
...
ones when computing a tree decomposition of a graph.
...
$\gamma$, we take the vertex such the sum of the next $k$ choices
...
times and reporting the best result we found. As the
...
Maximum-Cardinality Search heuristic. The heuristic order the vertices
of $G$ from $1$ to $n$ in the following ordering: We first put a
...
The algorithms expects two error parameters $r$ and $s$: the number of
restarts and the number of steps.  To find a tree decomposition the
algorithms starts upon a given permutation, then it will try to move a
...
An approximation algorithm as the one by Robertson and Seymour can be interesting in two ways: it
produces lower \emph{and} upper bounds at the same time, while also
provides a guarantee on its quality. This section list the
...
\chapter{Exact}
At heart of \Jdrasil{} is a collection of algorithms that compute
...
In addition to the different implemented algorithms, there is the
class \JClass{Exact} which provides stand alone access the
...
$n^{O(k)}$, this yield a way to compute exact tree decompositions. In
...
\end{lstlisting}
Or use one of \Jdrasil's start scripts, which automatically look for
...
compiles the C++-file under the assumption that there is an library
